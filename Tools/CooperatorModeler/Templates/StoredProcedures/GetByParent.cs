...
foreach (BaseTreeNode entityNode in Model.Children) {
    ...
    if (currentEntity.GenerateSpGetByParent) {
    ...
    foreach (BaseTreeNode propNode in currentEntity.Children) {
        ...
        if (currentProperty.IsEntity && (currentEntity.GenerateAsChildOf == currentProperty.RelatedTableName) && !spGenerated) {
        spGenerated = true;
%>
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[<%Response.Write(parameters["SpPrefix"]+currentEntity.Name+"_GetByParent");%>]') and OBJECTPROPERTY(id, N'IsProcedure') = 1) 
drop procedure [dbo].[<%Response.Write(parameters["SpPrefix"]+currentEntity.Name+"_GetByParent");%>]
<%Response.SaveBuffer(currentEntity.Name+"_GetByParentDrop");%>
--------------------------------------------------
...
--------------------------------------------------
CREATE PROCEDURE [dbo].[<%Response.Write(parameters["SpPrefix"]+currentEntity.Name+"_GetByParent");%>] 
<%
        string sep = "";
...
<%Response.SaveBuffer(currentEntity.Name+"_GetByParent");
}
}
}
}
